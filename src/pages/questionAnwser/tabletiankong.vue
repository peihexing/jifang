    ...
    <div class="anwser h5">
        <div class="anwser-box">
            ...
            <el-row class="radio-cut" v-for="(col, index) in data.content.colContent" :key="index">
                ...
                <div>
                    <el-col v-for="(item, idx) in data.options" class="el-col-w el-col-input" :key="idx" :span="Math.floor(24/data.options.length)">
                        <el-input v-model="result[index][idx]" @blur="handleResult(col.id, index)"></el-input>
                    </el-col>
                </div>
            </el-row>
        </div>
  ...
  methods: {
      handleResult(titleId,colIndex) {
          ...
          for(var i = 0; i< this.data.options.length; i++) {
              if (this.result[colIndex][i] == '' || this.result[colIndex][i] == undefined) {
                  flag = false
              }
          }
